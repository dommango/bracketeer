...
import { GroupStandings, Bracket } from "@/app/pool/[code]/Bracket";
...
const BASE = "/challenge/md3";

// The challenge Matches tab mirrors the pool's: the full tournament's fixtures
// (group + knockout), the live Golden Boot, and the betting markets — not just the
// 24 Match Day Pickem games. The viewer's MD3 scoreline overlay rides along where
// they predicted.
...
function GroupingToggle({ active }: { active: FixtureGrouping }) {
  const tab = (fx: FixtureGrouping, label: string) => {
    ...
    return (
      <Link
        href={`${BASE}/matches?view=groups&fx=${fx}#group-fixtures`}
        aria-current={on ? "page" : undefined}
...
export default async function Md3ChallengeMatchesPage({
  ...
  const hrefForMatch = (no: number) => `${BASE}/matches/${no}`;
      ...
      {active === "groups" ? (
        <>
          {bracket ? (
            <section>
              ...
              <div className="mt-2.5">
                <GroupStandings view={bracket} basePath={BASE} />
              </div>
            </section>
          ) : null}
          ...
          <section id="group-fixtures" className="scroll-mt-4">
            ...
            <div className="mt-3">
              {grouping === "city" ? (
                <VenueGrid basePath={BASE} venues={byCityVenues(groupRows)} />
              ) : (
                <MatchCenter
                  sections={
        ...
        <Scorers scorers={scorers} basePath={BASE} />
      ) : titleOdds.length > 0 || favorites.length > 0 ? (
        <div className="space-y-5">
          {titleOdds.length > 0 ? <ChampionshipOdds odds={titleOdds} basePath={BASE} /> : null}
          {favorites.length > 0 ? (
            <OddsBoard
              ...
              rows={favorites.map((f) => ({
                ...
                winProb: f.winProb,
                href: `${BASE}/players/${encodeURIComponent(f.playerName)}`,
              }))}
            />
          ) : null}
        </div>
