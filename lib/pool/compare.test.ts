...
import { knockoutDivergences, championStatus } from "./compare";
...
describe("championStatus", () => {
  it("returns an empty status when no champion is picked", () => {
    expect(championStatus(picks(), results())).toEqual({ code: null, name: null, alive: false });
  });

  it("reports the champion pick as alive while no results eliminate it", () => {
    const status = championStatus(picks({ knockout: { 104: "BRA" } }), results());
    expect(status).toMatchObject({ code: "BRA", alive: true });
